...
//char uart_tx_buffer[UART_BUFFER];
//unsigned int uart_tx_counter = 0;
...
char patterns[SLOTS] = {85,85,85,
                        ...
                        85,85,85,
                        85,85,85};
/*unsigned long sizes[SLOTS] = {4*Kbit,4*Kbit,4*Kbit,
                              4*Kbit,4*Kbit,4*Kbit,
                              4*Kbit,4*Kbit,4*Kbit,
                              4*Kbit,4*Kbit,4*Kbit};//*/
unsigned long sizes[SLOTS] = {16,16,16,
                              16,16,16,
                              16,16,16,
                              16,16,16};//*/
...
{         // writes a byte to serial comms
    ...
    UCA0TXBUF = data;
    /*uart_tx_buffer[uart_tx_counter] = data; // add to buffer
    uart_tx_counter++;  // increment buffer counter
    UCA0IFG |= UCTXIFG; // set tx flag*/
}
...
{
    char pattern = 85;  // checkerboard pattern
    ...
    if(data == pattern){uart_tx('1');}  // print 1 if data matches pattern
    else{uart_tx('0');} // print 0 if not
}
...
void fill(char chip, unsigned long size, char pattern)
// fill memory with given pattern
{
    unsigned long i;    // declare iterator
    set_write_mode(pattern);    // get ready to write pattern
    for(i = 0; i < size; i++)   // for each byte in size
    {
...
void FILL(unsigned long *size, char *pattern)
{
    unsigned char i;
    for(i = 0; i < SLOTS; i++)
    {
...
void read(char chip, unsigned long size)
...
void check(char chip, unsigned long size, char pattern)
...
void CHECK(unsigned long *size, char *pattern)
{
    uart_tx(':');
    uart_tx('\n');
    unsigned char i;
...
void scan(char chip, unsigned long size, char pattern)
...
void SCAN(unsigned long *size, char *pattern)
{
    uart_tx(':');
    unsigned char i;
    for(i = 0; i < SLOTS; i++)
...
void MENU(void)
{
    ...
    uart_tx_string("size    : read and set the size (in bytes) of all devices\n");
...
__interrupt void USCI_A0_ISR(void)
{
    switch(UCA0IV)
    {
        ...
        case USCI_UART_UCRXIFG: // RX flag
            ...
            uart_rx_counter++;
            break;
        case USCI_UART_UCTXIFG: // TX flag
            /*if(uart_tx_counter > 0)
            {
                UCA0TXBUF = uart_tx_buffer[0];
                unsigned int i;
                for(i = 0; i < uart_tx_counter; i++)
                {
